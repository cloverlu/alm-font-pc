        ...
        el-row(:gutter="20")
          el-col(:span="12")
            el-form-item(label="贷后检查模式 :" class="formItem2")
              el-input(v-model="form.checkModel" clearable)
          el-col(:span="12")
          ...
          el-col(:span="12")
            el-form-item(label="检查地点 :" class="formItem2")
              el-input(v-model="form.practicableCheckAddr" clearable)
          el-col(:span="12")
          ...
          el-col(:span="12")
            el-form-item(label="押品重估 :" class="formItem2")
              el-select(v-model="form.revalOfColl" style="width:100%" clearable)
                el-option(v-for="item in options" :key="item.value" :label="item.label" :value="item.value")
    //- 
    el-card(class='card')
      .cardTitle1
    ...
    el-card(class='card')
      ...
      el-form(:model="form" :inline="true" label-position="top" label-width="80px" size="mini")
        ...
        el-row(:gutter="20")
          el-col(:span="12")
            el-form-item(label="企业实际股权结构是否与上述工商信息网查询一致 :" class="formItem2")
              el-select(v-model="form.ownerStruSame" style="width:100%" clearable)
                el-option(v-for="item in options" :key="item.value" :label="item.label" :value="item.value")
          el-col(:span="12")
            el-form-item(label="企业所在行业是否发生重大不利变化 :" class="formItem2")
              el-select(v-model="form.IndustrycChangSiut" style="width:100%" clearable)
                el-option(v-for="item in options" :key="item.value" :label="item.label" :value="item.value")
        el-row(:gutter="20")
          el-col(:span="12")
            el-form-item(label=" " class="formItem2")
              el-input(v-model="form.ownerStruSameMsg" type="textarea" :rows="3" clearable)
          el-col(:span="12")
            el-form-item(label=" " class="formItem2")
              el-input(v-model="form.IndustrycChangSiutMsg" type="textarea" :rows="3" clearable)
        el-row(:gutter="20")
          el-col(:span="12")
            el-form-item(label="企业主营业务情况是否发生变更 :" class="formItem2")
              el-select(v-model="form.mainBusIsChanged" style="width:100%" clearable)
                el-option(v-for="item in options" :key="item.value" :label="item.label" :value="item.value")
          el-col(:span="12")
            el-form-item(label="企业是否有与主业无关的扩张计划 :" class="formItem2")
              el-select(v-model="form.planExpandSitu" style="width:100%" clearable)
                el-option(v-for="item in options" :key="item.value" :label="item.label" :value="item.value")
        el-row(:gutter="20")
          el-col(:span="12")
            el-form-item(label=" " class="formItem2")
              el-input(v-model="form.mainBusIsChangedMsg" type="textarea" :rows="3" clearable)
          el-col(:span="12")
            el-form-item(label=" " class="formItem2")
              el-input(v-model="form.planExpandSituMsg" type="textarea" :rows="3" clearable)
        el-row(:gutter="20")
          el-col(:span="12")
            el-form-item(label="企业生产经营场所是否发生或即将发生变动 :" class="formItem2")
              el-select(v-model="form.proAndOpeAbnormalSuit" style="width:100%" clearable)
                el-option(v-for="item in options" :key="item.value" :label="item.label" :value="item.value")
          el-col(:span="12")
            el-form-item(label="企业主要原材料或货物的采购成本是否明显上升 :" class="formItem2")
              el-select(v-model="form.purchaseCost" style="width:100%" clearable)
                el-option(v-for="item in options" :key="item.value" :label="item.label" :value="item.value")
        el-row(:gutter="20")
          el-col(:span="12")
            el-form-item(label=" " class="formItem2")
              el-input(v-model="form.proAndOpeAbnormalSuitMsg" type="textarea" :rows="3" clearable)
          el-col(:span="12")
            el-form-item(label=" " class="formItem2")
              el-input(v-model="form.purchaseCostMsg" type="textarea" :rows="3" clearable)
        el-row(:gutter="20")
          el-col(:span="12")
            el-form-item(label="企业生产经营是否出现异常情况(停产、半停产、销售大规模退货、员工数量骤减、劳资关系紧张、设备开工率不足、拖欠租金等) :" class="formItem2")
              el-select(v-model="form.orderDecline" style="width:100%" clearable)
                el-option(v-for="item in options" :key="item.value" :label="item.label" :value="item.value")
          el-col(:span="12" style='paddingTop:24px')
            el-form-item(label="企业销售是否出现异常情况(销售大规模退货、存货积压严重) :" class="formItem2")
              el-select(v-model="form.saleAbnormalSuit" style="width:100%" clearable)
                el-option(v-for="item in options" :key="item.value" :label="item.label" :value="item.value")
        el-row(:gutter="20")
          el-col(:span="12")
            el-form-item(label=" " class="formItem2")
              el-input(v-model="form.orderDeclineMsg" type="textarea" :rows="3" clearable)
          el-col(:span="12")
            el-form-item(label=" " class="formItem2")
              el-input(v-model="form.saleAbnormalSuitMsg" type="textarea" :rows="3" clearable)
        el-row(:gutter="20")
          el-col(:span="12")
            el-form-item(label="企业上下游核心客户是否发生重大变更 :" class="formItem2")
              el-select(v-model="form.chainChange" style="width:100%" clearable)
                el-option(v-for="item in options" :key="item.value" :label="item.label" :value="item.value")
          el-col(:span="12")
            el-form-item(label="制造型企业水、电、煤、气等资源消耗量是否较上年同期明显下降(降幅达30%) : :" class="formItem2")
              el-select(v-model="form.dailyCostDecline" style="width:100%" clearable)
                el-option(v-for="item in options" :key="item.value" :label="item.label" :value="item.value")
        el-row(:gutter="20")
          el-col(:span="12")
            el-form-item(label=" " class="formItem2")
              el-input(v-model="form.chainChangeMsg" type="textarea" :rows="3" clearable)
          el-col(:span="12")
            el-form-item(label=" " class="formItem2")
              el-input(v-model="form.dailyCostDeclineMsg" type="textarea" :rows="3" clearable)
        el-row(:gutter="20")
          el-col(:span="12")
            el-form-item(label="企业生产经营是否有安全隐患 :" class="formItem2")
              el-select(v-model="form.hiddenTroubleSitu" style="width:100%" clearable)
                el-option(v-for="item in options" :key="item.value" :label="item.label" :value="item.value")
          el-col(:span="12")
        ...
        el-row(:gutter="20")
          el-col(:span="12")
            el-form-item(label=" " class="formItem2")
              el-input(v-model="form.hiddenTroubleSituMsg" type="textarea" :rows="3" clearable)
          el-col(:span="12")
            el-form-item(label=" " class="formItem2")
        ...
        el-row(:gutter="20")
          el-col(:span="12")
            el-form-item(label="剔除季节性因素后的现金流是否有大幅下降(降幅超30%) :" class="formItem2")
              el-select(v-model="form.cashDecline" style="width:100%" clearable)
                el-option(v-for="item in options" :key="item.value" :label="item.label" :value="item.value")
          el-col(:span="12")
            el-form-item(label="现金流与营业收入是否基本匹配 :" class="formItem2")
              el-select(v-model="form.cashMatchesAndProAndOpe" style="width:100%" clearable)
                el-option(v-for="item in options" :key="item.value" :label="item.label" :value="item.value")
        el-row(:gutter="20")
          el-col(:span="12")
            el-form-item(label="上次检查(或调查)至本次检查期间的现金流 :" class="formItem2")
              el-input(v-model="form.cashLastToNow" type="textarea" :rows="3" clearable)
          el-col(:span="12")
            el-form-item(label="其他 :" class="formItem2")
              el-input(v-model="form.cashOtherMsg" type="textarea" :rows="3" clearable)
    //- 检查要点小结
    el-card(class='card')
      .cardTitle1
        span(class='blue')
      ...
      el-form(:model="form" :inline="true" label-position="top" label-width="80px" size="mini")
        el-row(:gutter="20")
          el-col(:span="12")
            el-form-item(label=" " class="formItem2")
              el-input(v-model="form.summaryForCheck" type="textarea" :rows="3" clearable)
        ...
        el-row(:gutter="20")
          el-col(:span="12")
            el-form-item(label="押品名称: " class="formItem2")
              el-input(v-model="item.assitName" clearable)
          el-col(:span="12")
            el-form-item(label="押品位置地址: " class="formItem2")
              el-input(v-model="item.assitAddr" clearable)
        el-row(:gutter="20")
          el-col(:span="12")
            el-form-item(label="首次评估情况: " class="formItem2")
              el-input(v-model="item.firstEstimateDate" clearable)
          el-col(:span="12")
            el-form-item(label="我行认定价值: " class="formItem2")
              el-input(v-model="item.firstEstimateValue" clearable)
          el-col(:span="12")
            el-form-item(label="抵质押率: " class="formItem2")
              el-input(v-model="item.firstMortAndpleRate" clearable)
        el-row(:gutter="20")
          el-col(:span="12")
            el-form-item(label="最近一次评估情况: " class="formItem2")
              el-input(v-model="item.LastEstimateDate" clearable)
          el-col(:span="12")
            el-form-item(label="我行认定价值: " class="formItem2")
              el-input(v-model="item.LastEstimateValue" clearable)
          el-col(:span="12")
            el-form-item(label="抵质押率: " class="formItem2")
              el-input(v-model="item.LastMortAndpleRate" clearable)
        el-row(:gutter="20")
          el-col(:span="12")
            el-form-item(label="最近一次评估情况: " class="formItem2")
              el-input(v-model="item.thisEstimateDate" clearable)
          el-col(:span="12")
            el-form-item(label="我行认定价值: " class="formItem2")
              el-input(v-model="item.thisEstimateValue" clearable)
          el-col(:span="12")
            el-form-item(label="抵质押率: " class="formItem2")
              el-input(v-model="item.thisMortAndpleRate" clearable)
          el-col(:span="12" style="marginTop:24px")
            el-button(@click="addDomain1" type='primary' v-if="index == 0") 新增
            el-button(@click="removeDomain1(item)" type='primary' v-if="index !== 0") 删除
        ...
        el-row(:gutter="20")
          ...
          el-col(:span="12")
            el-form-item(label="我行合作状态: " class="formItem2")
              el-input(v-model="item.CooperatStatus" clearable)
        el-row(:gutter="20")
          el-col(:span="12")
            el-form-item(label="风险分类: " class="formItem2")
              el-input(v-model="item.assitFiveClass" clearable)
          el-col(:span="12" style="marginTop:24px")
            el-button(@click="addDomain2" type='primary' v-if="index == 0") 新增
            el-button(@click="removeDomain2(item)" type='primary' v-if="index !== 0") 删除
      ...
      el-form(:model="form" :inline="true" label-position="top" label-width="80px" size="mini")
        el-row(:gutter="20")
          el-col(:span="12")
            ...
            el-form-item(label=" " class="formItem2")
              el-input(v-model="form.assitChangeSuit" type="textarea" :rows="3" clearable)
          el-col(:span="12")
            .cardTitle1
              span(class='title') 押品其他情况
            el-form-item(label=" " class="formItem2")
              el-input(v-model="form.assitOtherSuit" type="textarea" :rows="3" clearable)
          el-col(:span="12")
            .cardTitle1
              span(class='title') 押品检查小结
            el-form-item(label=" " class="formItem2")
              el-input(v-model="form.summaryForAssit" type="textarea" :rows="3" clearable)
